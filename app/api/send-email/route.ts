...
export async function POST(request: Request) {
    const body = await request.json();
    const { name, contactDetails, message } = body;

    try {
        await resend.emails.send({
            from: 'Your Website <onboarding@resend.dev>', // Use the default sender or your verified domain
            to: 'abdullatif.trefeifi@gmail.com',
            subject: `New Contact Form Submission from ${name}`,
            text: `
        Name: ${name}
        Contact Details: ${contactDetails}
